...
| `smtp-antispam` | host:port | active | Assess mail-bombing resilience (rate-limit) and spam/spoof filter effectiveness; at intrusive+ confirms an open-relay/spoof finding by delivering a single labelled probe message via full `DATA` (see [safety.md](safety.md)) | rate-limit absent or a spoofed mail is accepted (delivered, at intrusive+) |
...
| `cors-probe` | url / host:port | active | Detect permissive CORS (CWE-942): reflects an attacker Origin, tests `null`/wildcard/subdomain trust, and flags reflection combined with `Access-Control-Allow-Credentials: true` | Origin reflected (or `null`/`*`) with credentials, or a trusted-subdomain reflection |
| `waf-detect` | url / host:port | detective | Detect a WAF/IPS (Cloudflare/Akamai/AWS/Imperva/F5/Sucuri/… — CWE-693) by passive header/cookie fingerprint plus an active 3-payload challenge; scores confidence and warns that MITIGATED findings may be WAF-blocked | never (informational); reports the detected WAF at ≥50% confidence |
| `api-enum` | url / host:port | active | Enumerate the API surface (CWE-200): docs (Swagger/Redoc), OpenAPI/Swagger specs, GraphQL introspection, versioned REST roots, Spring Actuator/metrics/debug endpoints | an anonymously-readable spec or a sensitive management endpoint (`/actuator/env`, `/debug/pprof`) is reachable |
...
| `service-mesh-probe` | host:port / url | Detect service-mesh sidecars (Envoy/Istio) via admin ports and enumerate upstream clusters/routes; also queries Consul's HTTP API (`/v1/catalog/services`, `/v1/catalog/nodes`) for the control plane's own peer topology, queuing every disclosed node for follow-on probing; at intrusive+ re-fetches `/config_dump` and extracts embedded secrets (inline_string, PEM certificates) | sidecar admin port is exposed and upstream clusters are enumerable; or Consul catalog answers unauthenticated |
| `db-topology` | host:port | Database cluster/replica-set topology discovery: MongoDB isMaster/hello replica-set membership, Redis `CLUSTER NODES`/`INFO replication` peers, Elasticsearch `_cat/nodes` cluster membership — every peer beyond the scanned node is queued for follow-on probing | a multi-node cluster/replica-set is disclosed |
| `oob-management-probe` | host:port | Out-of-band management interface discovery: IPMI (RMCP presence ping), Redfish (`/redfish/v1/`), and iLO/iDRAC/Supermicro web-banner fingerprints; at intrusive+ probes `/redfish/v1/Systems` unauthenticated | a BMC/IPMI/Redfish interface is reachable; or system inventory readable with no auth |
| `iot-protocol-probe` | host:port | IoT/industrial-control protocol discovery: CoAP (bare GET), Modbus TCP (Read Holding Registers), OPC-UA (Hello/Ack handshake) — all read-only handshakes, no writes | a Modbus or OPC-UA endpoint answers (CoAP alone is informational) |
| `container-registry-enum` | cloud / host:port | Container registry enumeration: ECR `DescribeRepositories`/`GetRepositoryPolicy` (flags Principal:* public repos), GCP Artifact Registry per-project listing, and unauthenticated JFrog Artifactory `/artifactory/api/repositories` | repositories enumerated; or a public/unauthenticated repository listing confirmed |
...
| `broker-auth` | host:port | intrusive | Probe MQTT/AMQP brokers for anonymous or default auth; also checks the RabbitMQ Management HTTP API (`/api/overview`, port 15672) with `guest:guest` — a second, vendor-specific detection surface independent of the raw AMQP handshake; at proof+ sends a wildcard `SUBSCRIBE #` to the confirmed anonymous MQTT broker to capture all published messages | an anonymous/default-cred broker connection is accepted (AMQP, MQTT, or the management API) |
...
| `ssrf-probe` | url / host:port | intrusive | Generic SSRF (CWE-918) on URL/reference parameters: OOB callback (via `--oob-listen`), `file:///etc/passwd` read, cloud-metadata reach, and localhost response differential (de-dupes params, ≤`SSRF_MAX_PARAMS`) | an OOB callback fires, a local file/metadata is read, or the localhost probe changes the response |
| `nosqli-confirm` | url / host:port | intrusive | NoSQL injection (CWE-943): MongoDB `$ne` auth-bypass (JSON body + `[$ne]` query), `$where` differential, Elasticsearch wildcard expansion, Redis CRLF `CONFIG GET` — all confirmed by response differential vs a baseline | a payload flips a rejected request to authorised, or surfaces backend data/config |
| `dns-attack-probe` | host / host:port / url | intrusive | Active DNS attacks (CWE-350): per-NS zone transfer (AXFR via `dig`) and DNS rebinding detection (varying answers / TTL<5s). AXFR with RFC1918/internal records escalates to CRITICAL | a nameserver serves AXFR, or rebinding facilitators are present |
| `forbidden-bypass` | url / host:port | intrusive | 401/403 access-control bypass (CWE-284): path-normalisation tricks (`/.`, `//`, `..;/`, `%2f`, `%09`, case) and rewrite/trusted-IP headers (`X-Original-URL`, `X-Rewrite-URL`, `X-Forwarded-For: 127.0.0.1`, `X-Custom-IP-Authorization`) against forbidden endpoints. On success it publishes the now-reachable URL to the web-confirmation chain and (via `exploited=True`) auto-registers the host as a pivot | a forbidden endpoint returns 2xx through a bypass mutation |
| `graphql-mutation-executor` | url | intrusive | Invoke GraphQL mutations discovered via `graphql-probe`'s schema introspection (consumes `target.discovered_apis`), so the mutation surface is exercised even when discovery ran in an earlier/lower-intensity pass. Destructive-looking names are never invoked. | a discovered mutation executes without authentication |
| `grpc-method-invoker` | host:port | intrusive | Invoke gRPC methods discovered via `grpc-reflection`'s server-reflection listing (consumes `target.discovered_apis`) with `grpcurl` and an empty payload | a discovered public-looking method answers without authentication |
