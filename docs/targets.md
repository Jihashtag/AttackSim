...
| **netrange** | `--cidr` (IPv4 CIDR `10.0.0.0/24`, last-octet range `10.0.0.10-40`, full-IP range `10.0.0.250-10.0.1.10`, or IPv6 CIDR `2001:db8::/120` / `[2001:db8::]/120`; optional inline `:ports`), or positional; `+ --ports`, `--max-hosts` | `host-sweep`, `nmap-probe`, `nuclei-probe`, then a fan-out of the host:port modules across every **live** host |
